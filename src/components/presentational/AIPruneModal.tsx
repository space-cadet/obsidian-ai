...
import { MemoryOptimizer, ProgressUpdate } from "../../intelligence/MemoryOptimizer";
...
const AIPruneModal: React.FC<AIPruneModalProps> = ({ onClose, createOptimizer }) => {
	...
	useEffect(() => {
		...
		optimizer
			...
			.then((pruneResult) => {
				const savedKb = ((pruneResult.bytesBefore - pruneResult.bytesAfter) / 1024).toFixed(1);
				addLog(`Complete! Removed ${pruneResult.removed} duplicates, saved ~${savedKb} KB.`);
				setResult(`Removed ${pruneResult.removed} duplicates (${pruneResult.groups} groups). Kept ${pruneResult.kept} unique entries. Saved ~${savedKb} KB.`);
				setIsRunning(false);
			})
			.catch((err) => {
		...
		return () => {
			// Cleanup handled by cancel button
		};
	...
	return (
		<div className="chat-modal-overlay" onClick={!isRunning ? onClose : undefined}>
			<div className="chat-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "560px", width: "90vw" }}>
				...
				<div className="chat-modal-body">
					...
					{result && (
						<div style={{
							padding: "10px",
							background: "var(--background-modifier-success)" as any,
							borderRadius: "6px",
							fontSize: "0.9em",
							marginBottom: "12px",
						}}>
					...
					{error && (
						<div style={{
							padding: "10px",
							background: "var(--background-modifier-error)" as any,
							borderRadius: "6px",
							fontSize: "0.9em",
							marginBottom: "12px",
					...
					<div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
						{isRunning ? (
							<button
								...
								style={{ background: "var(--background-modifier-error)" as any, color: "var(--text-error)" }}
							>
								⏹ Cancel
							</button>
						) : (
							<button className="chat-btn chat-btn-primary" onClick={onClose}>
								Close
							</button>
						)}
					</div>
