		...
		vault: {
			...
			adapter: {
				exists: async (path: string) =>
					path in files ||
					path === ".obsidian/plugins/test-plugin/sessions",
				list: async (path: string) => {
					const files_list = Object.keys(files).filter((f) =>
						f.startsWith(path + "/"),
					);
					return {
						files: files_list.map((f) => f.split("/").pop()!),
						folders: [],
					};
				},
...
describe("SearchIndex", () => {
	it("builds index and searches across sessions", async () => {
		const files: Record<string, string> = {
			".obsidian/plugins/test-plugin/sessions/session-a.jsonl":
				JSON.stringify({
					id: "msg-1",
					role: "user",
				...
				"\n",
			".obsidian/plugins/test-plugin/sessions/session-b.jsonl":
				JSON.stringify({
					id: "msg-3",
					role: "user",
