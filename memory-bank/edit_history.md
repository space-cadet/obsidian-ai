...
*Last Updated: 2026-08-02 20:18:00 IST*

---

## 2026-08-02

#### 20:03 IST - Bug Fix: Desktop Chat View Duplication (Workspace Race)
- Modified `src/main.ts` - Added `requestAnimationFrame` guard in `activateChatView()` to prevent workspace restoration race creating duplicate leaves

#### 19:07 IST - Bug Fix: Desktop Chat View Duplication (React Render)
- Modified `src/views/ObsidianAIChatView.ts` - Added `this.contentEl.empty()` on open + `renderPending` guard to prevent concurrent renders
