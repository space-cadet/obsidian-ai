# Changelog
*Created: 2026-05-02 00:00:00 UTC*
*Last Updated: 2026-05-02 00:00:00 UTC*

*Tracks significant changes to the project. Newest first.*

---

## May 2026

### 2026-05-02 — Memory Bank Initialized [META-1]

- Created `memory-bank/` directory with all required subdirectories
- Initialized all 11 core memory bank files per integrated-rules-v6.12.md
- Documented project as InlineAI Plugin v1.2.4 (obsidian-ai)
- Branch: `claude/setup-memory-bank-Y9eIn`

---

## Project Version History (Pre-Memory-Bank)

| Version | Date | Notes |
|---------|------|-------|
| 1.2.4 | — | Bump version; current release |
| 1.2.3 | — | Previous release |
| — | — | Azure OpenAI support added |
| — | — | Focus guard for editor re-renders |
