...
- Synced chat sessions opening empty ("No messages" until reopened): a failed first read no longer permanently marks the session hydrated — it retries on next open and logs the error — T24 (`20ae983`).
- History menu showing "No messages" for synced conversations: unhydrated sessions now display their indexed message count ("N messages — open to view") with no file reads — T24 (`1fb9263`).
- Text-only sends after image attachments demanding an image-capable model: replayed history now maps image parts to an `[image attached]` placeholder; stored transcripts unchanged — T48b (`f812c3f`).
- Settings "Sync Now" button not showing the progress modal (command palette did) — T42g (`8fafd34`).
- Sync operations failing with `Maximum call stack size exceeded` on every path: `emitLog` self-recursion in the lifecycle wiring introduced by the observability hub — T42g (`5749148`).

### Notes

- `styles.css` is generated by `scripts/concat-styles.mjs` from the `styles/_*.css` partials — edit the partials, never the generated file — T66.
